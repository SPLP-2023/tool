// PDF Generation Functions with Aspect Ratio Preservation
function addImageToPDF(pdf, imageData, x, y, maxWidth, maxHeight) {
    if (imageData) {
        try {
            ...
            // Create a canvas to measure the image
            ...
            // Get dimensions (this works synchronously for base64 data)
            const originalWidth = img.naturalWidth || img.width || maxWidth;
            const originalHeight = img.naturalHeight || img.height || maxHeight;
            
            // If we can't get dimensions, fall back to a simple approach
            if (!originalWidth || !originalHeight || originalWidth === 0 || originalHeight === 0) {
                console.log('Using fallback dimensions');
                pdf.addImage(imageData, format, x, y, maxWidth, maxHeight * 0.75); // Slightly less height to avoid most distortion
                return maxHeight * 0.75 + 5;
            }
            ...
            console.log(`Original: ${originalWidth}x${originalHeight}, Final: ${finalWidth.toFixed(1)}x${finalHeight.toFixed(1)}`);
            ...
            // Fallback: use original fixed dimensions approach
            const format = imageData.includes('data:image/jpeg') ? 'JPEG' : 'PNG';
...
function addPageHeader(pdf, title) {
    // Add header logo if available - FIXED with max dimensions
    if (uploadedImages['headerLogoPreview_data']) {
        addImageToPDF(pdf, uploadedImages['headerLogoPreview_data'], 170, 10, 30, 20);
    }
...
function generatePDF() {
    ...
    if (uploadedImages['coverLogoPreview_data']) {
        const logoHeight = addImageToPDF(pdf, uploadedImages['coverLogoPreview_data'], 60, 20, 90, 50);
    ...
    // FIXED: Building image with proper aspect ratio
    if (uploadedImages['buildingImagePreview_data']) {
        const imageHeight = addImageToPDF(pdf, uploadedImages['buildingImagePreview_data'], 30, yPosition, 150, 120);
    ...
    if (selectedFailuresList.length > 0) {
        selectedFailuresList.forEach((failure, index) => {
            ...
            // Estimate space needed for this failure (simplified calculation)
            ...
            pdf.setFont(undefined, 'bold');
            pdf.setFontSize(8);
            pdf.setTextColor(40, 167, 69); // Green color for requirements
            ...
            // FIXED: Add failure image with proper aspect ratio
            if (failure.imageData) {
                ...
                if (imagesOnPage < maxImages) {
                    ...
                    currentY += 5;
                    const imageHeight = addImageToPDF(pdf, failure.imageData, currentX, currentY, 70, 50);
                    currentY += imageHeight;
                    imagesOnPage++;
                }
            }
            
            currentY += 10; // Space between failures
    ...
    if (uploadedImages['earthImagesPreview_data']) {
        ...
        images.forEach((imageData, index) => {
            if (imageData) {
                ...
                const yPos = yPosition + (row * 80); // Increased spacing for variable height images
                
                // FIXED: Earth test images with proper aspect ratio
                addImageToPDF(pdf, imageData, xPos, yPos, 80, 70);
                imagesOnCurrentPage++;
            }
        });
    }
    ...
    pdf.save(filename);
            ...
            if (originalWidth / maxWidth > originalHeight / maxHeight) {
                // Image is more constrained by width
                finalWidth = maxWidth;
                finalHeight = maxWidth / aspectRatio;
            } else {
                // Image is more constrained by height  
                finalHeight = maxHeight;
                finalWidth = maxHeight * aspectRatio;
            }
...
function addPageHeader(pdf, title) {
    // Add header logo if available - FIXED with max dimensions
    if (uploadedImages['headerLogoPreview_data']) {
        addImageToPDFSync(pdf, uploadedImages['headerLogoPreview_data'], 170, 10, 30, 20);
    }
...
function generatePDF() {
    ...
    // ==================== COVER PAGE ====================
    if (uploadedImages['coverLogoPreview_data']) {
        const logoHeight = addImageToPDFSync(pdf, uploadedImages['coverLogoPreview_data'], 60, 20, 90, 50);
        yPosition = 20 + logoHeight + 10;
    } else {
        yPosition = 30;
    }
    ...
    // FIXED: Building image with proper aspect ratio
    if (uploadedImages['buildingImagePreview_data']) {
        const imageHeight = addImageToPDFSync(pdf, uploadedImages['buildingImagePreview_data'], 30, yPosition, 150, 120);
        yPosition += imageHeight + 20;
    }
        ...
        selectedFailuresList.forEach((failure, index) => {
            ...
            if (failure.imageData) {
                ...
                if (imagesOnPage < maxImages) {
                    ...
                    pdf.text('Image:', currentX, currentY);
                    currentY += 5;
                    const imageHeight = addImageToPDFSync(pdf, failure.imageData, currentX, currentY, 70, 50);
                    currentY += imageHeight;
                    imagesOnPage++;
                }
            }
    ...
    if (uploadedImages['earthImagesPreview_data']) {
        ...
        images.forEach((imageData, index) => {
            if (imageData) {
                ...
                // FIXED: Earth test images with proper aspect ratio
                addImageToPDFSync(pdf, imageData, xPos, yPos, 80, 70);
                imagesOnCurrentPage++;
            }
        });
    }
