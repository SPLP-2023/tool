...
// Synchronous version for simple cases where we accept some distortion for layout simplicity
function addImageToPDFSync(pdf, imageData, x, y, maxWidth, maxHeight) {
    if (imageData) {
        try {
            // For now, let's use a simpler approach that works synchronously
            // We'll extract basic dimensions from the base64 data if possible
            const format = imageData.includes('data:image/jpeg') ? 'JPEG' : 'PNG';
            ...
            // Use natural dimensions if available, otherwise use reasonable defaults
            const originalWidth = tempImg.naturalWidth || 800;
            const originalHeight = tempImg.naturalHeight || 600;
            const aspectRatio = originalWidth / originalHeight;
